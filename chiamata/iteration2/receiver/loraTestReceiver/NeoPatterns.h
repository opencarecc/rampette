...
enum  pattern { NONE, RAINBOW_CYCLE, THEATER_CHASE, COLOR_WIPE, SCANNER, FADE, BLINK_RED, COLOR_WHEEL, FADETO, SPINNER };
...
class NeoPatterns : public Adafruit_NeoPixel
{
  public:
    ...
    void spinner(uint32_t color1, uint8_t interval, direction dir = FORWARD, int t = 3) {
    ...
    void spinnerUpdate() {
      ...
      for (int i = 0; i < numPixels(); i++) {
        ...
        if (i < tail) {
          ...
          int r = map(bri, 0, 255, 0, Red(Color1));
          int g = map(bri, 0, 255, 0, Green(Color1));
          int b = map(bri, 0, 255, 0, Blue(Color1));
